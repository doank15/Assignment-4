...
import { Chat, GMail, Person, Submit } from './Icons';
import { validateForm } from '../utils/formValidation';
import createMessage from '../api/create_message';

const Contact = () => {
    const initialFormData = {username: "", email: "", message: ""};
    const [formData, setFormData] = useState(initialFormData)
    const initialErrData = {nameError: "", emailError: "", messageError: ""};
    const [errData, setErrorData] = useState(initialErrData)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        validateForm(formData, setFormData, setErrorData, initialFormData, initialErrData)
        const data = {...formData};
        try {
            const response = await createMessage.add(data);
            alert("Insert message success: ", response.username)
        } catch (error) {
            console.log(error);
        }
    }
    ...
    return (
        <div name="Contact" className='pt-10 justify-center h-full min-h-screen w-full flex items-center bg-gradient-to-b to-gray-800 from-black'>
            <div className='section  w-1/2'>
                <Heading heading="Contact" secondHeading="Fill the form to get in touch with me" />
                <div className='flex justify-center items-center'>
                    <form onSubmit={handleSubmit} className='flex gap-3 flex-col w-full md:w-1/2'>
                        <div className=" w-full">
                            <div className='relative'>
                                <input type="text" name="username" placeholder="Enter Your Name" className={`peer form-input ${errData.nameError !== "" && "border-red-500"} `} value={formData.name} onChange={handleChange} />
